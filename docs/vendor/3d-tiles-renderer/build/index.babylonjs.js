import { T as E } from "./TilesRendererBase-BGxy2Uih.js";
import { L, g as O, r as j } from "./LoaderBase-CU5shB7w.js";
import { TransformNode as S } from "@babylonjs/core/Meshes/transformNode";
import { Matrix as u, Quaternion as W, Vector3 as i } from "@babylonjs/core/Maths/math.vector";
import { Frustum as A } from "@babylonjs/core/Maths/math.frustum";
import { Observable as N } from "@babylonjs/core/Misc/observable";
import { Plane as Q } from "@babylonjs/core/Maths/math.plane";
import { B as U } from "./B3DMLoaderBase-CMKvUOVT.js";
...
class V extends L {
  ...
  async parse(e, t, n) {
    ...
    h.addAllToScene();
    const p = h.rootNodes[0];
    p.rotationQuaternion = W.Identity();
    const m = p.computeWorldMatrix(!0);
    return o.multiplyToRef(m, P), P.decompose(p.scaling, p.rotationQuaternion, p.position), {
      scene: p,
      container: h,
      metadata: f
    };
...
const D = /* @__PURE__ */ u.Identity(), C = /* @__PURE__ */ new i(), B = /* @__PURE__ */ new Array(6).fill(null).map(() => new Q(0, 0, 0, 0));
class le extends E {
  /**
   ...
   */
  constructor(e, t) {
    super(e), this.scene = t, this.group = new S("tiles-root", t), this.checkCollisions = !1, this._upRotationMatrix = u.Identity(), this._observables = /* @__PURE__ */ new Map();
  }
  addEventListener(e, t) {
    this._observables.has(e) || this._observables.set(e, new N()), this._observables.get(e).add(t);
  }
  ...
  async parseTile(e, t, n, s, r) {
    const o = t.engineData, a = this.scene, c = O(s), f = this.fetchOptions, h = o.transform, p = this._upRotationMatrix;
    let m = null;
    const w = (j(e) || n).toLowerCase();
    switch (w) {
      case "b3dm": {
        const l = new H(a);
        l.workingPath = c, l.fetchOptions = f, l.adjustmentTransform.copyFrom(p), m = await l.parse(e, s);
        break;
      }
      case "gltf":
      case "glb": {
        const l = new V(a);
        l.workingPath = c, l.fetchOptions = f, l.adjustmentTransform.copyFrom(p), m = await l.parse(e, s, n);
        break;
      }
      default:
    ...
    const d = m.scene;
    if (d.setEnabled(!1), d.computeWorldMatrix(!0).multiply(h).decompose(d.scaling, d.rotationQuaternion, d.position), r.aborted) {
      m.container.dispose();
      return;
    }
    if (this.checkCollisions)
      for (const l of d.getChildMeshes())
        l.checkCollisions = !0;
    o.scene = d, o.container = m.container, o.metadata = m.metadata || null;
  }
  disposeTile(e) {
    super.disposeTile(e);
  ...
  calculateTileViewError(e, t) {
    ...
    if (w) {
      ...
      d = 2 / m[5] / h;
    this.group.getWorldMatrix().invertToRef(D), i.TransformCoordinatesToRef(o.globalPosition, D, C), A.GetPlanesToRef(o.getTransformationMatrix(!0), B);
    const k = B.map((R) => R.transform(D)), M = r.distanceToPoint(C);
    let v;
    w ? v = e.geometricError / l : v = M === 0 ? 1 / 0 : e.geometricError / (M * d);
